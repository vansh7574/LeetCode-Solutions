...
-- with totals as (
--     select p.product_id, (p.price * isNull(u.units, 0)) as total_amount, Sum(isNull(units,0)) over (partition by p.product_id) as total_units from Prices p Left Join UnitsSold u on p.product_id = u.product_id where u.purchase_date BETWEEN start_date and end_date
-- ),
-- product_totals as (
